...
void EnergyLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
	...
	for (int bottom_id = 0; bottom_id < bottom.size(); bottom_id++) {
		const Dtype* noise_param = bottom[bottom_id]->cpu_data();
				...
				for (int i = 0; i < param_size; i++) {
					switch(_model) {
					    ...
					    case EnergyLossParameter_ModelType_SQUARE_INVERSE: { 
					    	loss_val += _alpha / (noise_param[i] * noise_param[i]);
						} 	break;
		...
		top[0]->mutable_cpu_data()[0] += loss_val / param_size;
		LOG(INFO) << "Energy loss value for this bottom blob: " << loss_val / param_size;
	}
	LOG(INFO) << "top loss output: " << top[0]->mutable_cpu_data()[0];
	LOG(INFO) << "example noise param: " << bottom[0]->cpu_data()[0];
}
